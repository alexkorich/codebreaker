...
module Codebreaker
  describe Game do
    ...
    describe "#initialize" do
      ...
      it "restricts direct access to secret code" do
    ...
    describe "#submit_guess" do 
      ...
      msg="Please, enter 4-digits code with 1-6 numbers"
      context "returns message if guess is not valid" do
        it "rejects 11111" do
          expect(@game.submit_guess(11111)).to eq(msg)
        end
        it "rejects 111" do
          expect(@game.submit_guess(111)).to eq(msg)
        end
        it "rejects 1238" do
          expect(@game.submit_guess(1238)).to eq(msg)
        end
        it "rejects 'aaaa'" do
          expect(@game.submit_guess('aaaa')).to eq(msg)
        end
        it "rejects 'w111'" do
          expect(@game.submit_guess('w111')).to eq(msg)
        end
        it "rejects 1" do
          expect(@game.submit_guess(1)).to eq(msg)
        end
      end
      ...
      it "ttt" do
              @game.instance_variable_set(:@secret_code, "5551")
            expect(@game.submit_guess(1455)).to eq("+--")
          end
      end
    end
